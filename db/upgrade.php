...
function xmldb_congrea_upgrade($oldversion) {
    ...
    require_once($CFG->wwwroot . '/mod/congrea/locallib.php');
            ...
            if ($dbman->table_exists($table)) {
                ...
                if (!empty($congreadata)) {
                    foreach ($congreadata as $data) {
                        ...
                        mod_congrea_update_calendar($congreadata, $congreaold->starttime, $enddate, $congreaold->timeduration, $sessionid);
                    }
                }
            }
        }
